import { D as formatLongDate, c as academicYear, g as certDisplayName } from "./utils-BIiJ-s-U.mjs";
import { n as openHtmlDocument, t as logoDataUrls } from "./print-CifCDGub.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/certificates-BpqtJ9x9.js
function escapeHtml(s) {
	return s.replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll("\"", "&quot;").replaceAll("'", "&#39;");
}
function certificateSerial(volunteer, event) {
	return `NSS/${academicYear(event.date).replace("-", "")}/${volunteer.volunteerId}/${event.id.replace(/^evt-/, "").slice(0, 8).toUpperCase()}`;
}
var CERT_CSS = `
...
`;
function signatureShortName(fullName, fallback) {
	const clean = fullName.replace(/^dr\.\s*/i, "").trim();
	if (!clean) return fallback;
	return clean.split(/\s+/)[0] || fallback;
}
function principalSignature(fullName) {
...
function certificateSheet(opts) {
	...
	const template = opts.settings.certificateTemplateUrl ? `url('${opts.settings.certificateTemplateUrl.replaceAll("'", "%27")}')` : "url('/images/certificate-template.jpg')";
	const poSignatureImage = opts.settings.poSignature ? `<img src="${opts.settings.poSignature}" alt="Programme Officer signature" style="max-height:42px;max-width:140px;object-fit:contain;display:block;margin:0 auto 8px;" />` : `<div class="signature">${poSignature}</div>`;
	const principalSignatureImage = opts.settings.principalSignature ? `<img src="${opts.settings.principalSignature}" alt="Principal signature" style="max-height:42px;max-width:140px;object-fit:contain;display:block;margin:0 auto 8px;" />` : `<div class="signature">${principalSignatureText}</div>`;
	return `<div class="sheet" style="--certificate-template:${template}">
...
function wrapCertificateDocument(title, sheets) {
	return `<!doctype html>
<html>
<head>
  <meta charset="utf-8" />
...
async function htmlForCertificate(cert, volunteer, event, settings) {
	const logos = await logoDataUrls();
	return wrapCertificateDocument(`NSS Certificate — ${volunteer.fullName}`, certificateSheet({
		volunteer,
		event,
		settings,
...
async function htmlForCertificates(rows, settings) {
	const logos = await logoDataUrls();
	return wrapCertificateDocument("NSS Certificates", rows.map((row) => certificateSheet({
...
//#endregion
export { openCertificateDocument as i, htmlForCertificate as n, htmlForCertificates as r, certificateSerial as t };
