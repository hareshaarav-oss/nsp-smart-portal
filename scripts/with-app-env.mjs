...
function main(argv) {
  ...
  const executable = !command.includes("/") && !command.includes("\\") && existsSync(localBin) ? localBin : command;
  // Windows cannot execute a local `.cmd` shim directly through `spawn`.
  // Run it through the command shell so `npm run dev` also works with newer
  // Node releases, which otherwise throw `spawn EINVAL`.
