...
import type { IssuedCertificate, NssEvent, PortalSettings, Volunteer } from "./types";
...
export function certificateSerial(volunteer: Volunteer, event: NssEvent) {
  ...
  return `NSS/${year}/${volunteer.volunteerId}/${event.id.replace(/^evt-/, "").slice(0, 8).toUpperCase()}`;
}
...
function certificateSheet(opts: {
  ...
  issuedOn: string;
  logos: { college: string; nss: string };
}) {
...
export async function htmlForCertificate(
  ...
  return wrapCertificateDocument(
    ...
    certificateSheet({ volunteer, event, settings, issuedOn: cert.sentAt ?? cert.generatedAt, logos }),
  );
}
