...
const AVATAR = `<svg viewBox="0 0 80 96" xmlns="http://www.w3.org/2000/svg" class="photo">
...
export async function htmlForIdCard(volunteer: Volunteer, settings: PortalSettings) {
  ...
  const photo = volunteer.photoUrl
    ? `<img src="${volunteer.photoUrl}" alt="" class="photo" />`
    : AVATAR;
  ...
  const poSign = settings.poSignature
    ? `<img src="${settings.poSignature}" alt="Programme Officer signature" style="width:92px;height:34px;object-fit:contain;display:block;margin:0 auto 2px;" />`
    : `<div class="ink">Haresh</div>`;
  const principalSign = settings.principalSignature
    ? `<img src="${settings.principalSignature}" alt="Principal signature" style="width:92px;height:34px;object-fit:contain;display:block;margin:0 auto 2px;" />`
    : `<div class="ink right">T.J.Vyas</div>`;
